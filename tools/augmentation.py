...
parser.add_argument("--load_dir", required=True, help="path to the folder containing rgb images")
parser.add_argument("--save_dir", required=True , help="path to the foloder containing the GT images")
a = parser.parse_args()
...
def image_augmentation_zoom_out(load_img_fold,load_gt_fold, store_img_fold, store_gt_fold, zoom_rate):
    ...
    for i in orig_image:
        for j in zoom_rate:
            ...
            row_dif = int((row - row_out)/2)
            img_zoom_orig = pad0img(img_zoom_org,j , col_dif, row_dif)
            ...
            img_zoom_gt = pad0img(img_zoom_gtt,j , col_dif, row_dif)
...
if __name__ == "__main__":
    ...
    load_dir = a.load_dir
    load_dir_org = load_dir+"/Original"  
    load_dir_gt = load_dir+"/GT"  
    
    save_dir = a.save_dir 
    save_dir_org = save_dir + "/Original" 
    save_dir_gt = save_dir + "/GT" 
    
    # Save the orginal image in the new save directory 
    copy_tree(load_dir_org, save_dir_org)
    copy_tree(load_dir_gt, save_dir_gt)
    # Apply zoom in 
    print('zoom-ining the images ..............')
    image_augmentation_zoom_in(load_dir_org,load_dir_gt, save_dir_org, save_dir_gt, np.array([0.05, 0.1, 0.15, 0.2, 0.25, 0.3]))
    # Apply zoom out 
    print('zoom-outing the images .............')
    image_augmentation_zoom_out(load_dir_org,load_dir_gt, save_dir_org, save_dir_gt, np.array([0.05, 0.1, 0.15, 0.2, 0.25, 0.3]))    
    # Darken the images 
    print('darkening the images ...............')
    image_augmentation_darken(load_dir_org,load_dir_gt, save_dir_org, save_dir_gt, np.array([0.5,0.75]))
    # Brighten the images
    print('brightening the images .............')
    image_augmentation_brighten(load_dir_org,load_dir_gt, save_dir_org, save_dir_gt, np.array([1.25,1.5]))
    # rotate the images
    print('rotating the images ................')
    image_augmentation_rotation(load_dir_org, load_dir_gt, save_dir_org, save_dir_gt)
    print('Congratulations, augmenataion is done :)!')
